...
class get_wavecar(object):
    def __init__(self,
        ...
        self.eig, self.occ = self._get_eig_and_occ()
    ...
    def _get_eig_and_occ(self):
        ...
        for ispin in range(self.I_spin):
            for ikpt in range(self.N_kpt):
                ...
                """
                eig[ispin, ikpt] = np.sign(
                    eig_tmp[:, 0]) * np.abs(eig_tmp[:, 0] + eig_tmp[:, 1] * 1j)
    ...
    def get_fftgrid(self):
        ...
        phi12 = np.arccos(
            np.dot(self.bcell[0], self.bcell[1]) / bmag[0] / bmag[1])
        ...
        phi13 = np.arccos(
            np.dot(self.bcell[0], self.bcell[2]) / bmag[0] / bmag[2])
        ...
        phi23 = np.arccos(
            np.dot(self.bcell[1], self.bcell[2]) / bmag[1] / bmag[2])
    ...
    def get_single_coeff(self, ispin=1, ikpt=1, iband=1, norm=False):
        ...
        """
        N = 3 + (self.N_kpt * (ispin - 1) +
                 (ikpt - 1)) * (1 + self.N_Band) + (iband - 1)
        self._file_.seek(self._Recl_ * N, 0)
        coeff_tmp = np.fromfile(self._file_,
                                dtype=self._prec_coeff_,
                                count=self._Len_coeff_)
        if norm:
            coeff_tmp /= np.linalg.norm(coeff_tmp)
        return coeff_tmp

    def get_coeff_at_kpoints(self,
                             ispin=1,
                             ikpt=1,
                             start_band=1,
                             end_band=1,
        ...
        """
        N = 3 + (self.N_kpt * (ispin - 1) +
                 (ikpt - 1)) * (1 + self.N_Band) + (start_band - 1)
        self._file_.seek(self._Recl_ * N, 0)
        coeff_tmp = np.fromfile(self._file_,
                                dtype=self._prec_coeff_,
                                count=self._Len_coeff_ *
                                (end_band - start_band + 1)).reshape(
                                    ((end_band - start_band + 1), -1))
        if norm:
            coeff_tmp /= np.linalg.norm(coeff_tmp, axis=1)[:, np.newaxis]
        return coeff_tmp
        ...
        for ispin in range(self.I_spin):
            for ikpt in range(self.N_kpt):
                ...
                coeff_tmp = np.fromfile(self._file_,
                                        dtype=self._prec_coeff_,
                                        count=self.N_Band *
                                        self._Len_coeff_).reshape(
                                            (self.N_Band, self._Len_coeff_))
                self.coeff[ispin, ikpt] = coeff_tmp
        if norm:
            self.coeff = self.coeff / np.linalg.norm(
                self.coeff, axis=3)[:, :, :, np.newaxis]
        return self.coeff
...
def calc_single_tdm(wfc, ikpt, ispin1, iband1, ispin2, iband2, norm=True):
    """
...
def calc_tdm_in_kpoint(wfc, ikpt, ispin1, rband1, ispin2, rband2, norm=True):
    """
    ...
    dE = (wfc.eig[ispin1 - 1, ikpt - 1, rband1 - 1] -
          (wfc.eig[ispin2 - 1, ikpt - 1, rband2 - 1])[:, np.newaxis])
    igall = wfc.gvector[ikpt - 1]
    ...
    tdm = np.zeros((band_index2, band_index1, 4), dtype=np.complex128)
    tmp = coeff1[np.newaxis].conj() * coeff2[:, np.newaxis]
    overlap = tmp.sum(2)
    tmp_project_g = np.sum(tmp[:, :, :, np.newaxis] *
                           igall[np.newaxis, np.newaxis],
                           axis=2)
    if wfc.L_gam:
        tmp2 = coeff1[np.newaxis] * coeff2[:, np.newaxis].conj()
        tmp_project_g_r = np.sum(tmp2[:, :, :, np.newaxis] *
                                 igall[np.newaxis, np.newaxis],
                                 axis=2)
        tmp_project_g = (tmp_project_g - tmp_project_g_r) / 2
...
def calc_tdm_in_band(wfc, kpath, ispin1, iband1, ispin2, iband2, norm=True):
    """
    his module will calculate the Transition dipole moment 
    between multiple KS-band in specific kpoint
    according to the formula in:
    ...
    for ikpt in kpath:
        ...
        tdm[ikpt - 1], overlap[ikpt - 1] = wfc.calc_single_tdm(ikpt,
                                                               ispin1,
                                                               iband1,
                                                               ispin2,
                                                               iband2,
